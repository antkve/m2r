...
The original Ramsey theorem is a theorem about sets and their partitions. 
We adapt the statement from Ramsey's original paper (see \cite{Ramsey1930}):

\begin{theorem}[\textcite{Ramsey1930}]
  Let $\Gamma$ be an infinite set. For all $r,k\in\mathbb{Z}^+$, for 
  any partition of the set of $k$-dimensional subsets of $\Gamma$
  into $r$ classes $\{\mathcal{C}_{1},\cdots,\mathcal{C}_{r}\}$, there exists an infinite subset 
  $\Delta\subset\Gamma$ such that: 
  \begin{center}
  for some $i\in \{1,\cdots,r\}$, every $k$-dimensional subset of $\Delta$ is in $\mathcal{C}_{i}$
  \end{center}
\end{theorem}
...
partition. Now, partitioning is a natural operation that can be applied to a
vast class of objects; hence the scope of the theory is broad.  

 

We aim to instill an appreciation of the subject in the reader and to show how
it may be applied to a wide range of mathematical structures. We will also focus on the ability of the theory to
