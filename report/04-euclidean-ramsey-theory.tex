...
$\Gamma_{p}:=\{\Gamma_{1},\cdots,\Gamma{k}\}$ of $[n]$ induced by the equivalence relation:

$$i \sim j \iff \pi(x_{i}) = \pi(x_{j})$$

Order $[k]$ as follows: $$i\leq j \iff  \pi(x_{i}) \leq \pi(x_{j})$$
This defines a newly indexed partition $\Gamma^*_{p}$ of [n].

Now there exist infinitely many prime integers $p$ in $\mathbb{N}$, 
and hence the set $\{\Gamma^*_{p}\ : \ p$ is prime in $\mathbb{N}\}$
is infinite. Then, since there exist only finitely many possible
partitions of $[n]$, one such partition $\overline{\Gamma}$ must be shared
by infinitley many prime numbers.

By Lemma \ref{lem:vec} and properties of prime integers, $\overline{\Gamma}$ 
