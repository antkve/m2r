...
between combinatorial and geometrical properties of finite subsets of Euclidean 
...
coefficients in $\mathbb{Q}$. We say that $\Lambda$ is $k$-regular over 
$\mathbb{N}$ if, under any $k$-colouring of $\mathbb{N}$, there exists a 
monochromatic $x\in\mathbb{N}^n$ such that $\Lambda x  = 0$.
Then the matrix $\Lambda$ is Partition Regular (PR) over $\mathbb{N}$ if $\Lambda$ is 
...
\bigg|  \ \exists j 
\in [m]: \forall i<j, \ n_{i}= 0 \ \emph{and} \ n_{j}=c \ \emph{and}  
\ \forall i>j, \ |n_{i}|\leq p\bigg\} $$
and for a given $y\in \mathbb{N}^{m+1}$ the (m,p,c)-set is defined as:
$$S_{m,p,c}(y):=\bigg\{y.n \ | n\in N_{m,p,c}\bigg\}$$

Then $y$ is called a generator of the $(m,p,c)$-set.
\end{definition}
...
Rado's theorem translates a combinatorial 
property of the positive integers into an algebraic property of matrices over $\mathbb{Q}$. 
...
In this section show how a generalisation of Rado's theorem (\cite{Erdös1973}) 
...
\begin{theorem}[\textcite{Erdös1973}]
...
Our main tool in the proof is the following Lemma, which is a generalisation of Rado's theorem.
